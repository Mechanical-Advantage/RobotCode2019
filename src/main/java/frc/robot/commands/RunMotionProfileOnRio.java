...
/**
 * Runs a motion profile on the roboRIO.
 * 
 * dt of profile must be 0.02.
 * 
...
public class RunMotionProfileOnRio extends Command {
  ...
  protected void execute() {
    ...
    // Pathfinder native headings are in radians
    desiredHeading = Pathfinder.boundHalfDegrees(Pathfinder.r2d(leftFollower.getHeading() - initialProfileYaw));
    ...
    // Graphing
    if (RobotMap.tuningMode) {
      SmartDashboard.putString("Profile Position Graph",
          Robot.genGraphStr(leftFollower.getSegment().position,
      ...
      SmartDashboard.putNumber("Profile A output",
          kA.get() * ((leftFollower.getSegment().acceleration + rightFollower.getSegment().acceleration) / 2));
