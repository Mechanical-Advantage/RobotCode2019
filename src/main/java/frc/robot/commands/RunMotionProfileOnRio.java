...
/**
 * Runs a motion profile on the roboRIO. dt of profile must be 0.02. If using
 * absolute yaw, starting yaw must be close to first point heading. If running
 * backwards, profile should be generated as if going forwards.
 */
public class RunMotionProfileOnRio extends Command {
	...
	private TunableNumber kPAngle = new TunableNumber("Profile Angle P"); // standard is 0.8 * (1.0/80.0), add max
																			// velocity like: 0.8*60 * (1.0/80.0) (if
																			// maxVel is 60)
	...
	/*
	 * Tuning Notes: Tune wheelbase with gyro correction disabled until heading is
	 * about accurate D should be significantly lower than P Too much D will
	 * overcorrect, robot will go too fast, not much is needed, in my initial test,
	 * any causes problems
	 */

	public RunMotionProfileOnRio(Trajectory trajectory, boolean flipLeftRight, boolean absHeading, boolean backwards,
			boolean endConverge) {
		this(trajectory, flipLeftRight, absHeading, backwards,
				endConverge ? ConvergenceMode.ALWAYS : ConvergenceMode.NEVER);
	}

	public RunMotionProfileOnRio(Trajectory trajectory, boolean flipLeftRight, boolean absHeading, boolean backwards,
			ConvergenceMode endConverge) {
	...
	public RunMotionProfileOnRio(String filename, boolean flipLeftRight, boolean absHeading, boolean backwards,
			boolean endConverge) {
		this(filename, flipLeftRight, absHeading, backwards,
				endConverge ? ConvergenceMode.ALWAYS : ConvergenceMode.NEVER);
	}
	...
	private void initCommand() {
		...
		case ORIGINAL_ROBOT_2018:
			...
			break;
		case EVERYBOT_2019:
			// THese values are from NotBot, not tested
			kP.setDefault(10);
			kD.setDefault(0);
			AngleErrorThreshold.setDefault(1.5);
		...
		case ROBOT_2019_2:
			kP.setDefault(0);
			kD.setDefault(0);
			AngleErrorThreshold.setDefault(0);
			wheelbase.setDefault(0);
			kPAngle.setDefault(0);
			kPAngleAdjust.setDefault(0);
			kV.setDefault(0);
			kA.setDefault(0);
			tiltThreshold = 0;
		default:
	...
	// In separate function because this gets run during init if in tuning mode to
	// load new wheelbase
	...
	protected void initialize() {
		...
		// getHeading only updates when calling calculate so get it from the segment
		// instead
	...
	protected void execute() {
		...
		desiredHeading = Pathfinder.boundHalfDegrees(Pathfinder.r2d(leftFollower.getHeading() - initialProfileYaw)); // Pathfinder
																														// native
																														// headings
																														// are
																														// in
		...
		double turn = 0;
		// system so that if yaw is off, correct for that without oscillation of
		// position control
		if (leftFollower.isFinished()) {
		...
		// Graphing
		if (RobotMap.tuningMode) {
			SmartDashboard.putString("Profile Position Graph", Robot.genGraphStr(leftFollower.getSegment().position,
					Robot.driveSubsystem.getDistanceLeft() - initialPositionLeft, rightFollower.getSegment().position,
			...
			SmartDashboard.putNumber("Profile Current Position Left",
					Robot.driveSubsystem.getDistanceLeft() - initialPositionLeft);
			SmartDashboard.putNumber("Profile Current Position Right",
					Robot.driveSubsystem.getDistanceRight() - initialPositionRight);
			SmartDashboard.putString("Profile Heading Graph", Robot.genGraphStr(desiredHeading, gyroHeading));
			...
			SmartDashboard.putNumber("Profile P output",
					kP.get() * ((leftFollower.getLastError() + rightFollower.getLastError()) / 2));
			SmartDashboard.putNumber("Profile V output",
					kV.get() * ((leftFollower.getSegment().velocity + rightFollower.getSegment().velocity) / 2));
			SmartDashboard.putNumber("Profile A output", kA.get()
					* ((leftFollower.getSegment().acceleration + rightFollower.getSegment().acceleration) / 2));
	...
	protected boolean isFinished() {
		boolean tilted = Math.abs(Robot.ahrs.getRoll()) >= tiltThreshold
				|| Math.abs(Robot.ahrs.getPitch()) >= tiltThreshold;
		// current segment and heading should be same on left and right, only check one
		// At end of profile only correct angle
		return leftFollower.isFinished() && !(endConverge == ConvergenceMode.IF_FLAT && tilted)
	...
	protected void end() {
		...
		if (RobotMap.tuningMode) {
			System.out.printf("DLeft: %f, DRight: %f, Yaw: %f, Target Yaw: %f, Target DLeft: %f, Target DRight: %f\n",
					Robot.driveSubsystem.getDistanceLeft() - initialPositionLeft,
					Robot.driveSubsystem.getDistanceRight() - initialPositionRight, gyroHeading, desiredHeading,
					leftFollower.getSegment().position, rightFollower.getSegment().position);
			...
			SmartDashboard.putNumber("Profile Final Error Right", Robot.driveSubsystem.getDistanceRight()
					- initialPositionRight - rightFollower.getSegment().position);
			SmartDashboard.putNumber("Profile Final Error Yaw",
					gyroHeading - Pathfinder.boundHalfDegrees(Pathfinder.r2d(leftFollower.getHeading())));
		}
	}
	...
	// based on DistanceFollower, adds getLastError, will track last point instead
	// of returning 0
	private class CustomDistanceFollower {
		 ...
		 */
		// modified to work with my modified calculate that will not advance segment
		// beyond end (added -1)
